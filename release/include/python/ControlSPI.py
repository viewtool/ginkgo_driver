...
def VSI_WriteBytes(DevType, DevIndex, SPIIndex, pData, Len):
    try:
        ...
        print('%s: %s'%(args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_WriteBytes(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pData, Len)
# Read data from SPI
...
def VSI_BlockWriteBytes(DevType,DevIndex,SPIIndex,pWriteData,BlockSize,BlockNum,IntervalTime):
    try:
        ...
        print('%s: %s'%(args.__class__.__name__, args))
        exit()
    return VSI_BlockWriteBytes(c_int(DevType),c_int(DevIndex),c_int(SPIIndex),pWriteData,c_ushort(BlockSize),c_ushort(BlockNum),c_uint(IntervalTime))
# Block mode read
def VSI_BlockReadBytes(DevType,DevIndex,SPIIndex,pReadData,BlockSize,BlockNum,IntervalTime):
    try:
        ...
        print('%s: %s'%(args.__class__.__name__, args))
        exit()
    return VSI_BlockReadBytes(c_int(DevType),c_int(DevIndex),c_int(SPIIndex),pReadData,c_ushort(BlockSize),c_ushort(BlockNum),c_uint(IntervalTime))
...
def VSI_BlockWriteReadBytes(DevType,DevIndex,SPIIndex,pWriteData,WriteBlockSize,pReadData,ReadBlockSize,BlockNum,IntervalTime):
    try:
        ...
        print('%s: %s'%(args.__class__.__name__, args))
        exit()    
    return VSI_BlockWriteReadBytes(c_int(DevType),c_int(DevIndex),c_int(SPIIndex),pWriteData,c_ushort(WriteBlockSize),pReadData,c_ushort(ReadBlockSize),c_ushort(BlockNum),c_uint(IntervalTime))